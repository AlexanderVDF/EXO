...
void ServiceSupervisor::retryOrFail(const QString &name)
{
    ...
    QTimer::singleShot(delay, this, [this, name]() {
        ...
        if (e.process && e.process->state() != QProcess::NotRunning) {
            e.process->terminate();
            if (!e.process->waitForFinished(3000))
                e.process->kill();
        }
        launchService(name);
    });
...
void ServiceSupervisor::cleanupProbe(const QString &name)
{
    ...
    // Pas de close() : il provoque un wildcard warning via QWebSocket::close()
    // deleteLater suffit — la probe est déjà retirée de m_probes, les timers
    // stoppés, donc aucun callback ne peut accéder à l'objet avant sa destruction
    if (it->client)  { it->client->deleteLater(); }
...
{
    // Nettoyer les probes — stopper timers AVANT de supprimer le client
    for (auto it = m_probes.begin(); it != m_probes.end(); ++it) {
        if (it->timeout) { it->timeout->stop(); delete it->timeout; }
        if (it->poll)    { it->poll->stop();    delete it->poll; }
        if (it->client)  { delete it->client; }
    }
    m_probes.clear();
    ...
    for (const QString &name : m_registry.serviceNames()) {
        ...
        if (entry.process && entry.process->state() != QProcess::NotRunning) {
            ...
            if (!entry.process->waitForFinished(5000))
                entry.process->kill();
        }
    }
}
