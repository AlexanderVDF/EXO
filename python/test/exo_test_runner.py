...
async def start_missing_services(down_services: list[str]) -> list[str]:
    ...
    for name in down_services:
        ...
        try:
            ...
            env.setdefault("EXO_XTTS_MODELS", r"D:\EXO\models\xtts")
    ...
    if started:
        ...
        for attempt in range(10):
            ...
            if still_down:
                print(f"  {_yellow(f'⚠ Timeout: {', '.join(still_down)} toujours DOWN après 5s')}")
...
async def main_loop(max_loops: int, timeout_ms: int, autoheal: bool = False) -> bool:
    ...
    for loop_num in range(1, max_loops + 1):
        ...
        if autoheal and not healed_once:
            ...
            if down:
                print(f"\n  {_cyan(f'[autoheal] {len(down)} service(s) DOWN: {', '.join(down)}')}")
