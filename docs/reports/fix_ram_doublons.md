> 🧭 [Index](../README.md) → [Rapports](../README.md#-rapports-de-correction--reports) → fix_ram_doublons.md
...
- [1. Diagnostic Initial](#1-diagnostic-initial)
  - [Processus détectés avant nettoyage](#processus-détectés-avant-nettoyage)
  - [Cause racine](#cause-racine)
- [2. Corrections Appliquées](#2-corrections-appliquées)
  - [A. Protection singleton — `python/shared/singleton_guard.py`](#a-protection-singleton-pythonsharedsingleton_guardpy)
  - [B. Microservices patchés (7/7)](#b-microservices-patchés-77)
  - [C. Script de nettoyage — `scripts/auto_kill_zombies.py`](#c-script-de-nettoyage-scriptsauto_kill_zombiespy)
  - [D. Optimisation VS Code — `.vscode/settings.json`](#d-optimisation-vs-code-vscodesettingsjson)
- [3. Résultats du Nettoyage](#3-résultats-du-nettoyage)
  - [Zombies éliminés](#zombies-éliminés)
  - [État final des ports](#état-final-des-ports)
- [4. Bilan RAM](#4-bilan-ram)
- [5. Validation Anti-Doublon](#5-validation-anti-doublon)
  - [Test singleton guard](#test-singleton-guard)
  - [Test auto_kill_zombies.py (dry-run post-nettoyage)](#test-auto_kill_zombiespy-dry-run-post-nettoyage)
- [6. Fichiers Modifiés/Créés](#6-fichiers-modifiéscréés)
  - [Créés](#créés)
  - [Modifiés](#modifiés)
...
1. **Utiliser `scripts/auto_kill_zombies.py --kill`** avant chaque session de développement pour nettoyer d'éventuels
orphelins
2. **Ne jamais tuer les processus venv "stub"** (4 MB, `.venv\Scripts\python.exe`) — ce sont les parents des vrais
processus ; les tuer tue aussi le service
3. **Privilégier `launch_all`** depuis VS Code pour les lancements groupés — le singleton guard empêchera les doublons
