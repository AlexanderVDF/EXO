...
\* Les PrivateBytes incluent les buffers Vulkan GPU mappés en mémoire virtuelle — la RAM physique consommée est le WorkingSet.
...
La latence de 672 ms est mesurée sur un signal sine non-vocal. Sur une vraie utterance vocale courte (ex: "Jarvis, météo"), la latence effective sera comparable ou inférieure car le modèle `medium` traite plus rapidement les segments courts.
...
2. **Modèle large-v3** : Le fichier `ggml-large-v3.bin` (2,95 Go) peut être conservé pour un éventuel retour arrière ou supprimé pour récupérer de l'espace disque.

3. **Monitoring** : Le log amélioré du stt_server affiche désormais `STT model: medium (1463MB) — device: vulkan — beam_size: 3` à chaque démarrage pour faciliter le diagnostic.
