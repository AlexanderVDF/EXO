...
private slots:
    ...
    void criticalTimeout_activatesSafeBoot()
    {
        ...
        // Attendre que le timeout de 2s se déclenche pour les critiques
        // Le timer interne est 2000ms — on attend un peu plus
        QTRY_VERIFY_WITH_TIMEOUT(ctrl.isSafeBootEnabled(), 3000);
    ...
    void nonCriticalTimeout_doesNotBlockBoot()
    {
        ...
        // websearch et news vont timeout → Degraded, pas Failed
        QTRY_VERIFY_WITH_TIMEOUT(ctrl.degradedCount() >= 2, 3000);

        // Safe Boot ne devrait PAS être activé (tous les critiques sont OK)
        QVERIFY(!ctrl.isSafeBootEnabled());
    }

    // ── 3. Service lent → timeout → Safe Boot ──
    void slowCriticalService_triggersSafeBoot()
    {
        ...
        // "system" ne répond pas → timeout 2s → Safe Boot
        QTRY_VERIFY_WITH_TIMEOUT(spyActivated.count() >= 1, 3000);
        QVERIFY(ctrl.isSafeBootEnabled());
    }
