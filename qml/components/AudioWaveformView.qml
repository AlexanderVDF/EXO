...
Item {
    ...
    Timer {
        ...
        onTriggered: {
            ...
            // Smooth interpolation between current and target samples
            if (root.targetSamples.length > 0) {
                ...
                if (curr.length !== n) {
                    curr = new Array(n)
                    for (var k = 0; k < n; ++k) curr[k] = 0.0
                }

                var lerp = root.smoothFactor
                var newSamples = new Array(n)
                for (var i = 0; i < n; ++i) {
                    var c = i < curr.length ? curr[i] : 0.0
                    var t = i < tgt.length  ? tgt[i]  : 0.0
                    newSamples[i] = c + (t - c) * lerp
                }
                root.currentSamples = newSamples
            }

            // Decay towards zero when inactive
            if (!root.active && root.currentSamples.length > 0) {
                var decay = root.currentSamples
                var hasEnergy = false
                var decayed = new Array(decay.length)
                for (var d = 0; d < decay.length; ++d) {
                    decayed[d] = decay[d] * 0.92
                    if (Math.abs(decayed[d]) > 0.001) hasEnergy = true
                }
                root.currentSamples = decayed
                if (!hasEnergy) root.currentSamples = []
            }

            // Request repaint on active canvases
            if (root.active) {
